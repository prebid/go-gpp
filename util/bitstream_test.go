...
func TestParseByte4(t *testing.T) {
	...
	assertStringsEqual(t, "ParseByte4 expected 4 bits to start at bit 46, but the byte array was only 6 bytes long (needs second byte)", err.Error())

	b, err = ParseByte4(testdata, 80)
	assertStringsEqual(t, "ParseByte4 expected 4 bits to start at bit 80, but the byte array was only 6 bytes long", err.Error())
...
func TestParseByte8(t *testing.T) {
	...
	assertStringsEqual(t, "ParseByte8 expected 8 bits to start at bit 11, but the byte array was only 2 bytes long", err.Error())

	b, err = ParseByte8([]byte{0x44, 0x76}, 18)
	assertStringsEqual(t, "ParseByte8 expected 8 bits to start at bit 18, but the byte array was only 2 bytes long", err.Error())
...
func TestParseUInt12(t *testing.T) {
	...
	assertStringsEqual(t, "ParseUInt12 expected a 12-bit int to start at bit 44, but the byte array was only 6 bytes long", err.Error())

	i, err = ParseUInt12(testdata, 40)
	assertStringsEqual(t, "ParseUInt12 expected a 12-bit int to start at bit 40, but the byte array was only 6 bytes long", err.Error())

	i, err = ParseUInt12(testdata, 37)
	assertStringsEqual(t, "ParseUInt12 expected a 12-bit int to start at bit 37, but the byte array was only 6 bytes long", err.Error())
...
func TestParseUInt16(t *testing.T) {
	...
	assertStringsEqual(t, "ParseUInt16 expected a 16-bit int to start at bit 44, but the byte array was only 6 bytes long", err.Error())

	i, err = ParseUInt16(testdata, 40)
	assertStringsEqual(t, "ParseUInt16 expected a 16-bit int to start at bit 40, but the byte array was only 6 bytes long", err.Error())
